...
export class GridsterPush {
    ...
    pushItems(direction: string, disable?: boolean): boolean {
        if (this.gridster.$options.pushItems && !disable) {
            ...
            this.iteration = 0;
            // If direction is from south, we don't want to push items until $item.y hits the targetRow.
            if (direction === this.fromSouth && this.gridsterItem.$item.y > this.findTargetRow()) {
                pushed = false;
            } else {
                // Now if $item.y hits the targetRow (or if the direction is not fromSouth) then move forward with pushing.
    ...
    findTargetRow(): number {
        // Find the targetRow where we want to start push item.
        // If dragging item from south, the targetRow is the next biggest y before started y position.
        let targetRow = 0;
        for (const gridItem of this.gridster.grid) {
            if (gridItem.item.y < this.gridsterItem.item.y && gridItem.item.y > targetRow) {
                targetRow = gridItem.item.y;
            }
        }
        return targetRow;
    }
    ...
    private push(gridsterItem: GridsterItemComponentInterface, direction: string): boolean {
        ...
        for (; i < conflicts.length; i++) {
            ...
            // New push logic if an item is dragged from south
            if (direction === this.fromSouth) {
                if (gridsterItem.item.y > itemCollision.item.y && this.trySouth(itemCollision, gridsterItem)) {
