...
export class GridsterUtils {
  ...
  static checkDragHandleClass(target: HTMLElement, current: HTMLElement, dragHandleClass: string, ignoreContentClass): boolean {
    ...
    static checkContentClass(target: HTMLElement, current: HTMLElement, contentClasses: string|string[]): boolean {
        if (!target || target === current) {
            return false;
        }
        // @ts-ignore
        if (target.hasAttribute('class')) {
            const cssClasses = target.getAttribute('class').split(' ');
            if (!Array.isArray(contentClasses)) {
                contentClasses = [contentClasses];
            }
            for (const ignoredClass of contentClasses) {
                if (cssClasses.indexOf(ignoredClass) > -1) {
                    return true;
                }
            }
        }
        // @ts-ignore
        return GridsterUtils.checkContentClass(target.parentNode, current, contentClasses);
    }
