...
import {FactorBarSpriteletLauncherService} from './factor-bar-spritelet-launcher.service';
...
describe('FactorBarSpriteletLauncherService', () => {
    let service: FactorBarSpriteletLauncherService;
    ...
    beforeEach(() => {
        ...
        service = TestBed.inject(FactorBarSpriteletLauncherService);
    ...
    it('should be created', () => {
        ...
        expect(service.getSpriteletActionKey()).toBe(WidgetConfigType.FACTOR_GRAPHING_BAR_CHART.toString());
    });

    it('should launch a factor bar chart from a column', () => {
        service.launchSpritelet(widget, spriteletColumnEvent);
        const widgetResult = WorkspaceStore.getCurrentReport().widgets[0];

        expect(widgetResult.configType).toBe(WidgetConfigType.FACTOR_GRAPHING_BAR_CHART);
    ...
    it('should launch a factor bar chart from a row', () => {
        ...
        expect(metaDataColumns[0].columnKey).toBe('rfv_exp_port_35');
        expect(metaDataColumns[1].columnKey).toBe('rfv_exp_bench_756');
        const path = (widgetResult.dataStore.metaData.inputs.get(FactorPathInput.configType) as FactorPathInput).path;
        expect(path.length).toBe(2);
        expect(path[0].level).toBe('_ROOT_');
        expect(path[0].value).toBe('PEP');
        expect(path[1].level).toBe('level-1');
        expect(path[1].value).toBe('STYLE');
    });
});
