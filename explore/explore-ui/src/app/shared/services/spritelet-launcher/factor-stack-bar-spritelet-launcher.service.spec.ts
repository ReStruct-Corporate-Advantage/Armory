...
import {FactorStackBarSpriteletLauncherService} from '@services/spritelet-launcher/factor-stack-bar-spritelet-launcher.service';
import {WorkspaceStore} from '../../../stores';
import {WidgetConfigType, WidgetInputType} from '@blk/explore-ui-core';
import {ColumnSet} from '@blk/explore-ui-column-option';

describe('FactorStackBarSpriteletLauncherService tests', () => {
    let service: FactorStackBarSpriteletLauncherService;
    ...
    beforeEach(() => {
        ...
        service = TestBed.inject(FactorStackBarSpriteletLauncherService);
    ...
    it('should be created', () => {
        ...
        expect(service.getSpriteletActionKey()).toBe(WidgetConfigType.FACTOR_GRAPHING_STACK_BAR_CHART.toString());
    });

    it('should launch a factor stack bar chart from a column', () => {
        service.launchSpritelet(widget, spriteletColumnEvent);
        const widgetResult = WorkspaceStore.getCurrentReport().widgets[0];

        expect(widgetResult.configType).toBe(WidgetConfigType.FACTOR_GRAPHING_STACK_BAR_CHART);
        ...
        expect(metaDataColumns[1].columnKey).toBe('rfv_exp_bench_756');

        expect(widgetResult.dataStore.metaData.inputs.has(BarChartAdditionalSettings.configType)).toBe(true);
        const barChartAdditionalSettings = widgetResult.dataStore.metaData.inputs.get(BarChartAdditionalSettings.configType) as BarChartAdditionalSettings;
        expect(barChartAdditionalSettings.isStacked).toBeTruthy();
        expect(barChartAdditionalSettings.stackByImmediateChild).toBeTruthy();
        expect(barChartAdditionalSettings.showSelected).toBeFalsy();
        expect(barChartAdditionalSettings.selectedAsMeasureSeries).toBeTruthy();
        expect(barChartAdditionalSettings.selectedChartType).toBe(ChartType.LINE);
        expect(barChartAdditionalSettings.selectedChartMarkerSymbol).toBe(ChartMarkerSymbol.CIRCLE);
    });
});
