...
import {NOTIFICATION_SERVICE_TOKEN, PortGroupSummaryChartLevel, PortGroupSummaryChartType, TelemetryService, WidgetConfigType, WidgetInputType} from '@blk/explore-ui-core';
...
describe('PgsBarChartSpriteletLauncherService', () => {
    let service: PgsTsChartSpriteletLauncherService;
    ...
    beforeEach(() => {
        ...
        service = TestBed.inject(PgsTsChartSpriteletLauncherService);
    ...
    it('should be created', () => {
        ...
        expect(service.getSpriteletActionKey()).toBe(TabularWidgetConstants.PGS_TS_CHART_SPRITELET.ACTION_KEY.toString());
    });
    ...
    it('should launch a pgs ts chart from a row', () => {
    ...
    it('should launch a pgs ts chart from with column level breakdowns', () => {
        ...
        expect(metaDataColumns.slice(1, metaDataColumns.length).every(col => col.optionValues.length === 0)).toBeTruthy();
        expect(metaDataColumns.length).toBe(3);
        expect(metaDataColumns[0].columnKey).toBe('portfolio');
    });
