...
const ArmsCategory = props => {
  const {clientRect, context, expanded, isSearched, node, prevKey, renderArmory, root, setExpanded, variant} = props;
    ...
    <ul className={`c-ArmsCategory overflow-auto${expanded === false ? " collapsed" : ""}`}>
      ...
      {node && node.length > 0 ? node.map((category, key) =>
        <li key={prevKey ? prevKey + "-" + key : key} className="c-Aside__list-item">
          {context === "editor" ? <StaticArmament category={category} index={prevKey ? prevKey + "-" + key : "" + key} recursiveRenderer={renderArmory} />
