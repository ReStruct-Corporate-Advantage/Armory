...
import {
  ...
  setComponentsConfig,
} from "../../pages/ComponentCreator/actions";
...
import {
  getPresentComponentsConfig,
  getLayout,
  getPreviousLayout,
  getArmory,
} from "../../pages/ComponentCreator/selectors";
...
const ComponentContainer = (props) => {
  ...
  useTraceUpdate(props, "ComponentContainer");
  ...
  useEffect(() => {
    ...
    for (var i = 0; i < verticalCellCount; i++) {
      for (var j = 0; j < horizontalCellCount; j++) {
        cellsRenew.push({
          borderBottom:
            i === verticalCellCount - 1 ? "1px solid darkgrey" : "none",
          borderLeft: "1px solid darkgrey",
          borderRight:
            j === horizontalCellCount - 1 ? "1px solid darkgrey" : "none",
  ...
  const cellRenders =
    ...
    cells.length > 0 &&
    cells.map((cell, key) => (
      <span key={key} className="position-absolute" style={cell} />
    ));
  const items = componentsConfig.components[0].descriptor.children;
  const componentRenders = compGen
    .iterateAndGenerateWithConfig(
      items,
      comContainerRef,
      "",
      selectedComponent,
    ...
    .map((componentObj) => componentObj.item);
  ...
  return (
    <div
      className="c-ComponentContainer position-relative"
      ref={comContainerRef}
      onWheel={touchHandler}
    >
      {isDevMode && (
        <div
          className="c-ComponentContainer__layout h-100 position-relative"
          ref={ref}
        >
          {cellRenders}
        </div>
      ...
      <div
        className="c-ComponentContainer__renders position-absolute"
        ref={drop}
      >
        {componentRenders.length > 0 ? (
          componentRenders
        ) : (
          <h1 className="drop-zone-msg" style={{ fontFamily: "monospace" }}>
            Drop Zone!
          </h1>
        )}
      </div>
